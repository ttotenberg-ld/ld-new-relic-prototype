...
# Patched copy for the demo: the darkly-nr-integration branch references an
# undefined `logger` and an unimported `newrelic.agent` inside the hook, so
# flag data never attaches to spans. Fixed names only; logic unchanged.
import logging
    ...
    class NewRelicHook(Hook):
        ...
        def after_evaluation(self, series_context, data, detail):
            try:
                ...
                _logger.debug("Attaching data to NR span")
                attrs = {
                ...
                if getattr(detail, "value", None) is not None:
